...
function ChoroplethGlobeComponent() {
  ...
  useEffect(() => {
    if (globeref.current) {
      ...
      globeref.current.pointOfView({ altitude: 2 }, 5000);
      globeref.current.controls().enableZoom = false;
    }
  }, []);

  const colorScale = resolvedTheme === 'dark' ? d3.scaleSequentialSqrt(d3.interpolateRgb('purple', 'magenta')) : d3.scaleSequentialSqrt(d3.interpolateRdYlGn) 
